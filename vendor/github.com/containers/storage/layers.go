...
func (r *layerStore) ApplyDiffWithDiffer(to string, options *drivers.ApplyDiffOpts, data interface{}, differ drivers.DifferFunction) (*drivers.DriverWithDifferOutput, error) {
	ddriver, ok := r.driver.(drivers.DriverWithDiffer)
	if !ok {
	...
	if to == "" {
		output, err := ddriver.ApplyDiffWithDiffer("", "", options, data, differ)
		return &output, err
	}
	...
	output, err := ddriver.ApplyDiffWithDiffer(layer.ID, layer.Parent, options, data, differ)
	if err != nil {
		return nil, err
	}
